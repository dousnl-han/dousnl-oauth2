...
public class RecRecoveryExcelUtil {
    private static final Logger logger = LoggerFactory.getLogger(RecRecoveryExcelUtil.class);
    public static String NO_DEFINE = "no_define";//未定义的字段
    public static String DEFAULT_DATE_PATTERN = "yyyy-MM-dd HH:mm:ss";//默认日期格式
    public static String DATE_PATTERN = "yyyy-MM-dd";//默认日期格式
    public static int DEFAULT_COLOUMN_WIDTH = 17;
    public static  String[] p_static;
    public static String[] h_static;
    public static DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");

    public static DateTimeFormatter dateTimeFormatter2 = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
    public static ZoneId zone = ZoneId.systemDefault();
    public static String IMPORT_EXCEL_CELL_TYPE_DATE_FORMAT_STR = "yyyy-MM-dd HH:mm:ss";//excel导入时间格式
    /**
    ...
    public static void exportExcel(String title, Map<String, String> headMap, JSONArray jsonArray, String datePattern, int colWidth, OutputStream out) {
        if (datePattern == null) datePattern = DATE_PATTERN;
        // 声明一个工作薄
...
//        cellStyle.setVerticalAlignment(HSSFCellStyle.VERTICAL_CENTER);
    ...
    public static void downloadExcelFile(String[] titles, Map<String,String>[] headMaps, JSONArray[] ja, HttpServletResponse response) throws Exception {
        ...
        SXSSFWorkbook wb = RecRecoveryExcelUtil.exportExcelX(titles, headMaps, ja, null, 0);
        try {
...
//                cellNum = row.getLastCellNum();
                ...
                for (int index = 0; index < cellNum; index++) {
                    ...
                    String[] prores = parseProperty(i + 1, String.valueOf(value==null?"":value), dataFormat);
                    if (prores.length == 2) {
                        ...
                        if (targetPd != null) {
                            ...
                            if (value != null && !value.equals("")) {
                                value = typeCast(writeMethod.getParameterTypes(),value.toString(),i,rowMsg);
                                writeMethod.invoke(rowData, value);
                                logger.info("Type:::" + targetPd.getPropertyType().getTypeName() + "__" + writeMethod.getName() + " Value:::" + value.toString());
                            }else {
                                logger.warn("Type:::" + targetPd.getPropertyType().getTypeName() + "__" + writeMethod.getName() + " Value:::" + value);
                            }
                        }
                    }
                }
    ...
    private static Object typeCast(Class<?>[] parameterTypes,String value,int i,StringBuffer rowMsg){
        ...
        }else if(parameterTypes[0].equals(ZonedDateTime.class)){
            return parseZoneDateTimeFromImportExcelDateCell(value,i,rowMsg);
        }else if(parameterTypes[0].equals(Timestamp.class)){
            return Timestamp.from(parseZoneDateTimeFromImportExcelDateCell(value,i,rowMsg).toInstant());
        }else if(parameterTypes[0].equals(Date.class)){
            return Date.from(parseZoneDateTimeFromImportExcelDateCell(value,i,rowMsg).toInstant());
        }else if(parameterTypes[0].equals(LocalDate.class)){
            return LocalDate.from(parseZoneDateTimeFromImportExcelDateCell(value,i,rowMsg).toInstant());
        }else if(parameterTypes[0].equals(LocalDateTime.class)){
            return LocalDateTime.from(parseZoneDateTimeFromImportExcelDateCell(value,i,rowMsg).toInstant());
        }else{
            return value;
        }
    }

    private static ZonedDateTime parseZoneDateTimeFromImportExcelDateCell(String value,int i,StringBuffer rowMsg) {
        Date cellDate = null;
        try {
            cellDate = DateUtils.parseDate(value,new String[]{IMPORT_EXCEL_CELL_TYPE_DATE_FORMAT_STR});
        } catch (ParseException e) {
            e.printStackTrace();
            rowMsg.append("第"+i+"行日期格式不正确\\r\\n");
        }
        return ZonedDateTime.ofInstant(cellDate.toInstant(),zone);
    }
    ...
    private static String getCellStringValue(Cell cell) {
        ...
        if (cell != null) {
            ...
            if(HSSFCell.CELL_TYPE_NUMERIC == cell.getCellType() && HSSFDateUtil.isCellDateFormatted(cell)){
                IMPORT_EXCEL_CELL_TYPE_DATE_FORMAT_STR = getExcelCellDateFormatStr(cell);
                cellValue = DateFormatUtils.format(cell.getDateCellValue(), IMPORT_EXCEL_CELL_TYPE_DATE_FORMAT_STR);
    ...
    private static String getExcelCellDateFormatStr(Cell cell) {
        StringBuffer formatBuf = new StringBuffer();
        String excelDateFormat = cell.getCellStyle().getDataFormatString().toUpperCase();
        if(excelDateFormat.contains("Y")){
            formatBuf.append("yyyy-");
            if(excelDateFormat.contains("M")){
                formatBuf.append("MM-");
                if(excelDateFormat.contains("D")){
                    formatBuf.append("dd");
                    if(excelDateFormat.contains("H")){
                        formatBuf.append(" HH:");
                        if(excelDateFormat.contains("M")){
                            formatBuf.append("mm:");
                            if(excelDateFormat.contains("S")){
                                formatBuf.append("ss");
                            }
                        }
                    }
            ...
            if(excelDateFormat.contains("M")){
                if(excelDateFormat.contains("D")){
                    formatBuf.append("MM-dd ");
                    if(excelDateFormat.contains("H")){
                        formatBuf.append("HH:mm");
                        if(excelDateFormat.contains("S")){
                            formatBuf.append("ss");
                        }
                    }
        ...
        for (int i = 0; i < columnNum; i++) {
            ...
            if (cell != null) {
                ...
                if (format != null) {
                    ...
                    throw new ServiceException("请根据正确模板导入");
                }
            }
        }
        return headMap;
    ...
    public static class DataFormat {
        ...
        public DataFormat(String property, String column, boolean isRequired, String format, String message,String dataFormart) {
            this.property = property;
            this.column = column;
            this.isRequired = isRequired;
            this.format = format;
